...
class FirstAvailable():
    ...
    def select_variable(self, grid):
        # Implementing the first available heuristic
...
class MRV():
    ...
    """
    def select_variable(self, grid):
        # Implementing the mrv heuristic
        imin = grid.get_width() + 1
        jmin = grid.get_width() + 1
        min_dom = 11
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) == 1:
                    continue
                if len(grid.get_cells()[i][j]) < min_dom:
                    min_dom = len(grid.get_cells()[i][j])
                    imin = i
                    jmin = j
        if (min_dom < 11):
           return (imin, jmin)
...
class AC3:
    ...
    def pre_process_consistency(self, grid):
        ...
        """
        # Implementing the code for making the CSP arc consistent as a pre-processing step; this method should be called once before search
    ...
    def consistency(self, grid, Q):
        ...
        """
        # Implementing the domain-dependent version of AC3.
        while len(Q) > 0:
            var = Q.pop()
            ival = var[0]
            jval = var[1]
            variables_assigned, failure = self.remove_domain_row(grid, ival, jval)
            if failure:
                return not failure
            else:
                for var_a in variables_assigned:
                    Q.add((var_a[0], var_a[1]))

            variables_assigned, failure = self.remove_domain_column(grid, ival, jval)
            if failure:
                return not failure
            else:
                for var_a in variables_assigned:
            ...
            variables_assigned, failure = self.remove_domain_unit(grid, ival, jval)
            if failure:
                return not failure
            else:
                for var_a in variables_assigned:
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        """
        # Implementing the Backtracking search.
        ...
        for d in domains:
            var_Q = set([var])
            copy_t = grid.copy()
            copy_t.get_cells()[var[0]][var[1]] = d

            if (ac3.consistency(copy_t, var_Q)):
                copy_g = copy_t.copy()
...
running_time_first_available = np.zeros(95)
running_time_mrv = np.zeros(95)
cnt = 0
print("Solving FirstAvailable...")
...
for p in problems:
    ...
    ac3 = AC3()
    g = ac3.pre_process_consistency(g)
    FirstAvl = FirstAvailable()
    bt = Backtracking()
    gout = bt.search(g, FirstAvl)
    end_time = time.time()
    running_time_first_available[cnt] = end_time - start_time
    cnt = cnt + 1
...
for p in problems:
    ...
    MRV_H = MRV()
    bt = Backtracking()
    gout = bt.search(g, MRV_H)
    end_time = time.time()
    running_time_mrv[cnt] = end_time - start_time
    cnt = cnt + 1
